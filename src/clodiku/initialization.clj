(ns clodiku.initialization
  ...
  (:require [clodiku.components :as comps]
            ...
            [clodiku.equipment.weapon-functions]))
...
(defn init-player [sys]
          (let [player (be/create-entity)
            ...
            (-> sys
                (be/add-entity spear)
                (be/add-component spear (comps/->EqItem {(comps/eq-stats :damage) 5 :slot (comps/eq-slots :held)} ))
                (be/add-component spear (comps/->EqWeapon
                                          (Rectangle. (float 0) (float 0) (float 4) (float 4))
                                          clodiku.equipment.weapon-functions/spear-function))
